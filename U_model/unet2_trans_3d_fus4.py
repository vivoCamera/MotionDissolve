...
class EEC_Cross(nn.Module):
    def __init__(self, dim, bias=False):
        super(EEC_Cross, self).__init__()


        self.Conv=nn.Conv2d(2*dim, dim, kernel_size=1, bias=bias)

        self.trans=nn.Conv2d(3*dim, dim, kernel_size=1, bias=bias)

        self.CA=ChannelAttention(dim)
        self.ViTFus = TransformerBlock_Cross(
                    dim=dim,
                    num_heads=4,
    ...
    def forward(self, f_img, f_event, Mask):
        ...
        # 下面对应的是Eq4
        F_fus = self.ViTFus(F_event, f_img)
        F_cat = torch.cat([F_event, f_img, F_fus], dim=1) 
        F_conv = self.trans(F_cat) + F_fus
        ...
        F_conv =  F_conv * w_c 
        # F_event=F_event * w_c
        F_event = self.Conv(torch.cat([F_event, F_conv], dim=1)) + F_event
...
class Restoration(nn.Module):
    ...
    def __init__(self, 
        ...
        self.EEC_1=EEC_Cross(self.channels[0])
        self.EEC_2=EEC_Cross(self.channels[1])
...
if __name__ == "__main__":
    ...
    Total MACs: 175.36 G, Params: 20.89 M【64 64 128 128】
