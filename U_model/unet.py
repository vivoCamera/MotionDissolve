...
class Restoration(nn.Module):
    ...
    def __init__(self, inChannels_img, inChannels_event,outChannels, args,ends_with_relu=False):
        ...
        self.encoder_event_2=EN_Block(inChannels_event, self.channels[1],0)
    ...
    def forward(self, input_img, input_event):
        ...
        '''
        # 
        # M0 = torch.clamp(self.blur(self.blur(torch.sum(torch.abs(input_event), axis = 1, keepdim = True), 
        #                                      kernel = 7, channels = 1), kernel = 7, channels = 1), 0, 1)

        M0 = torch.clamp(self.blur(self.blur(torch.sum(torch.abs(input_event), axis = 1, keepdim = True), 
                                             kernel = 7, channels = 1), kernel = 7, channels = 1), 0, 1)
        
        # print('M0.shape:{}'.format(M0.shape)) # M0.shape:torch.Size([16, 1, 128, 128])
        ...
        ####  feature extraction
        # TODO mapping img: 3 - 64; event: 8 - 64
        ...
        # print('event_1.shape:{}'.format(event_1.shape)) # event_1.shape:torch.Size([16, 64, 128, 128])
...
if __name__ == "__main__":
    ...
    model_restoration = Restoration(3, 8, 3, opt)
    ...
    '''
    Total MACs: 82.01 G, Params: 10.60 M
    '''
